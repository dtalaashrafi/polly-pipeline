...
static cl::opt<std::string>
    PipelineBuild("polly-pipeline-build",
                  cl::desc("Build pipeline pattern between iterations of "
...
// pipeline (*)
// TODO: change the name, location, use two isl_pw_multi_aff_list s to
// show multiple in and out dependencies in the general case. For now,
// use a bool to show in and out dependencies.
struct dummys {
  int in_out;
  isl_pw_multi_aff *d;
};
...
void IslNodeBuilder::createForPipeline(isl::ast_node For, bool MarkParallel) {
  ...
  for (int dim = 0; dim < dim_count; dim++) {
    ...
    isl_pw_aff *pw_dim = isl_pw_multi_aff_get_pw_aff(out_depend_copy2, dim);
    ...
    isl_pw_multi_aff_free(out_depend_copy2);
  ...
  for (int i = 0; i < depend_num; i++) {
    ...
    int in_dim_count = isl_pw_multi_aff_dim(in_depend_i, isl_dim_out);
    in_depend_dims.push_back(in_dim_count);

    auto in_depend_i_copy = isl_pw_multi_aff_copy(in_depend_i);

    isl_ast_build *in_dependExpBuild =
        isl_ast_build_from_context(isl_pw_multi_aff_domain((in_depend_i_copy)));

    for (int dim = 0; dim < in_dim_count; dim++) {
      auto in_depend_i_copy2 = isl_pw_multi_aff_copy(in_depend_i);
      auto in_dependExprBuild_copy2 = isl_ast_build_copy(in_dependExpBuild);

      isl_pw_aff *pw_dim = isl_pw_multi_aff_get_pw_aff(in_depend_i_copy2, dim);
      isl_ast_expr *astExpr = isl_ast_build_expr_from_pw_aff(
          in_dependExprBuild_copy2, isl_pw_aff_copy(pw_dim));

      temp_vec.push_back(astExpr);

      isl_pw_aff_free(pw_dim);
      isl_pw_multi_aff_free(in_depend_i_copy2);
      isl_ast_build_free(in_dependExprBuild_copy2);
    }
    in_depend_dim_vecs.push_back(temp_vec);

    isl_pw_multi_aff_free(in_depend_i);
    isl_ast_build_free(in_dependExpBuild);
  }
  ...
  std::unique_ptr<ParallelLoopGenerator> ParallelLoopGenPtr;
  ParallelLoopGenPtr.reset(new ParallelLoopGeneratorGOMP(Builder, LI, DT, DL));
  ...
  for (int num = 0; num < depend_num; num++) {
    ...
    in_depend_expr_builder.push_back(temp);
  }
