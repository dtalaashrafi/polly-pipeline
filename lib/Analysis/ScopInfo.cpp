...
isl::map get_one_pipeline_relation(isl::map rmap, isl::map wmap) {
...
isl::map get_blocks_per_one_pipeline_relation(isl::set s, isl::map m,
                                              int choose) {
  isl::set D;
  if (choose == 0)
    D = m.domain();
...
void Scop::getPipelineGraph() {
  ...
  std::vector<MemoryAccess *> write_list;
  PipelineGraph edges;

  // iterating over all memory access
  for (auto acc = s_acc_func.begin(); acc < s_acc_func.end(); acc++) {
    if ((*acc).get()->isRead())
      read_list.push_back((*acc).get());
    else // may read and must read
      write_list.push_back((*acc).get());
  }

  for (auto read : read_list)
    for (auto write : write_list)
      if (read->getAccessRelation().get_tuple_name(isl::dim::out) ==
              write->getAccessRelation().get_tuple_name(isl::dim::out) &&
          read->getAccessRelation().get_tuple_name(isl::dim::in) !=
              write->getAccessRelation().get_tuple_name(isl::dim::in))
...
void Scop::getPipelineRelations() {
  ...
  for (auto v : pipeline_graph) {
    auto v1 = v.first;
    auto v2 = v.second;
    auto rd_stmt = v1->getStatement();
    auto wr_stmt = v2->getStatement();
    auto rd_dom_map = rd_stmt->getDomain().flatten_map();
