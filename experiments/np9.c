...
void task_func(gmp_data **A , gmp_data **B, gmp_data **C, gmp_data **D , mpz_t m)
{
    ...
    for(int i=0 ; i<M-1 ; i++)
        for(int j=1 ; j<M/2-1 ; j++)
            B[i*N+j] = compute_function_3(B[i*M+j], B[i*M+(j-1)], B[(i+1)*M+(j+1)],A[(i)*N+(2*j)]);

    for(int i=0 ; i<Q-1 ; i++)
        for(int j=1 ; j<Q/2-1 ; j++)
            C[i*N+j] = compute_function_4(C[i*M+j], C[i*M+(j-1)], C[(i+1)*M+(j+1)], B[(i)*N+(2*j)], A[(i)*N+(j)]); 
	
  /* for(int i=0 ; i<Q-1 ; i++)
       for(int j=1 ; j<Q/2-1 ; j++)
          D[i*N+j] = compute_function_4(D[i*M+j], D[i*M+(j-1)], D[(i+1)*M+(j+1)], B[(i)*N+j], A[(i)*N+(2*j)]);	*/       
