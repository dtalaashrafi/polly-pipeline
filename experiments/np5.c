...
gmp_data *compute_function_2(gmp_data *res, gmp_data *a, gmp_data *b)
{
    for(int i=0 ; i<SIZE ; i++)
    {
...
void task_func(gmp_data **A , gmp_data **B, gmp_data **C, gmp_data **D , mpz_t m)
{
    ...
    for(int i=0 ; i<N-1 ; i++)
        for(int j=0 ; j<N-1 ; j++)
            A[i*N+j] = compute_function_2(A[i*N+j], A[i*N+(j+1)], A[(i+1)*N+(j+1)]);
    ...
    for(int i=0 ; i<(Q/2)-1 ; i++)
        for(int j=0 ; j<(Q/2)-1 ; j++)
            C[i*N+j] = compute_function_4(C[i*M+j], C[i*M+(j+1)], C[(i+1)*M+(j+1)], B[(2*i)*N+(2*j)]  ,A[(i)*N+(j)]);        
