...
public:
  ...
  // pipeline (*)
  void deployPipelineExecution(Function *SubFn, Value *SubFnParam, Value *LB,
                               Value *UB, Value *Stride, Value *OutDepend, 
                               Value *OutIndex, Value *InDepend, Value *InIndex ,
                               Value *Size, Value *DependNum) override;
  ...
  // pipeline (*)
  std::tuple<Value *, Function *> createSubFnPipeline(Value *Stride, AllocaInst *Struct,
                                                      SetVector<Value *> UsedValues,
                                                      ValueMapT &VMap) override;

  // pipeline(*) //depend
  std::tuple<Value *, Function *> createGetDependFn(Value *LB, Value *UB, Value *Stride, 
                                                    SetVector<Value *> &UsedValues,
                                                    ValueMapT &Map, BasicBlock::iterator *LoopBody) override;
  ...
  // pipeline (*)
  Value *createCallGetWorkItemPipeline(Value *LBPtr, Value *UBPtr); 
  ...
  // pipeline (*) //depend
  Function *prepareDependFnDefinition(Function *F) override;
