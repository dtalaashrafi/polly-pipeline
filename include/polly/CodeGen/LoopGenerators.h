...
public:
  ...
  // pipeline (*)
  Value *createPipelineLoop(Value *LB, Value *UB, Value *Stride,
                            SetVector<Value *> &Values, ValueMapT &VMap,
                            BasicBlock::iterator *LoopBody, Value *OutDepend,
                            Value *OutIndex, Value *InDepend, Value *InIndex,
                            Value *DependNum);                          
...
public:
  ...
  // pipeline (*)
  virtual void deployPipelineExecution(Function *SubFn, Value *SubFnParam,
                                       Value *LB, Value *UB, Value *Stride, Value *OutDepend,
                                       Value *OutIndex, Value *InDepend, Value *InIndex ,Value *Size,
                                       Value *DependNum) = 0;
  ...
  // pipeline (*) //depend
  virtual Function *prepareDependFnDefinition(Function *F) = 0;
  ...
  // pipeline (*)
  virtual std::tuple<Value *, Function *>
  createSubFnPipeline(Value *Stride, AllocaInst *Struct, SetVector<Value *> UsedValues,
              ValueMapT &VMap) = 0;     

    // pipeline(*)
  virtual std::tuple<Value *, Function *> 
  createGetDependFn(Value *LB, Value *UB, Value *Stride, SetVector<Value *> &UsedValues,
                    ValueMapT &Map, BasicBlock::iterator *LoopBody) = 0;   

  // pipeline (*)
  Function *createDependFnDefinition();  
};
} // end namespace polly
#endif
